...
- O pacote atual ainda não é instalador para usuário final e exige associação manual do driver. A aquisição TWAIN está verificada em clientes x86/x64 para prévia Gray por transferência native e memory, e em x64 para página inteira Color native. A fonte do provedor WIA 2.0 e do instalador GUI foi adaptada, mas não compilada; não há provedor WIA verificado, portanto aplicativos exclusivamente WIA ainda não descobrem o HR7. Acompanhe os bloqueios e requisitos em [Windows/Installer/README.md](Windows/Installer/README.md); não distribua essa fonte como instalador pronto.
