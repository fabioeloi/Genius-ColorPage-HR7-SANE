    ...
    _In_    BYTE        *pWiasContext)
{
    ...
    if(pWiasContext)
    {
        ...
        if(SUCCEEDED(hr))
        {
        // This device is a single flatbed scanner. Do not advertise the
        // sample's simulated feeder, duplex, film, or storage capabilities.
        LONG lDocumentHandlingCapabilities = FLAT;
        ...
        if(SUCCEEDED(hr))
        {
        LONG lDocumentHandlingStatus = FLAT_READY;
        ...
        if (SUCCEEDED(hr))
        {
            BSTR bstrFirmware = SysAllocString(L"Not reported");
            if ( bstrFirmware )
            {
            ...
            UINT        uiResourceID)
{
    ...
    if((pWiasContext)&&(hInstance))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            LONG lPixWidth          = 0;
            LONG lPixHeight         = 0;
            LONG lXResolution       = 300;
            LONG lYResolution       = 300;
            // The Plustek backend reports a 215 x 297 mm HR7 scan area.
            // WIA_IPS_MAX_*_SIZE uses 1/1000-inch units; advertise the
            // conservative whole-unit conversion so full-page selections do
            // not extend past the backend's physical-coordinate limits.
            LONG lHorizontalSize    = 8464;
            LONG lVerticalSize      = 11692;
            LONG lMinHorizontalSize = 1; //0.001"
            LONG lMinVerticalSize   = 1; //0.001"
            LONG lItemType          = 0;

            lPixWidth = (lHorizontalSize * lXResolution + 999) / 1000;
            lPixHeight = (lVerticalSize * lYResolution + 999) / 1000;
            lXExtent = lPixWidth;
            lYExtent = lPixHeight;
            ...
            if((lXExtent)&&(lYExtent)&&(lXResolution)&&(lYResolution)&&(lHorizontalSize)&&(lVerticalSize))
            {
                ...
                if((uiResourceID == IDB_FLATBED) || (uiResourceID == IDB_FILM))
                {
                    ...
                    if(SUCCEEDED(hr))
                    {
                        LONG lSegmentation = WIA_DONT_USE_SEGMENTATION_FILTER;
                ...
                if(SUCCEEDED(hr))
                {
                    ...
                    LONG lCurrentIntentValidValues  = WIA_INTENT_IMAGE_TYPE_COLOR | WIA_INTENT_IMAGE_TYPE_GRAYSCALE | WIA_INTENT_MINIMIZE_SIZE | WIA_INTENT_MAXIMIZE_QUALITY;
                ...
                if(SUCCEEDED(hr))
                {
                    ...
                    lXResolutionArray.Append(75);
                    lXResolutionArray.Append(150);
                    lXResolutionArray.Append(600);
                ...
                if(SUCCEEDED(hr))
                {
                    ...
                    lYResolutionArray.Append(75);
                    lYResolutionArray.Append(150);
                    lYResolutionArray.Append(600);
